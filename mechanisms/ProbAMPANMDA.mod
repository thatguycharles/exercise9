...
Modified by Aman Aberra to add deterministic flag
ENDCOMMENT
...
NEURON {
    THREADSAFE
        ...
        RANGE Use, u, Dep, Fac, u0, mg, NMDA_ratio, AMPA_scaling
        RANGE i, i_AMPA, i_NMDA, g_AMPA, g_NMDA, g, e
        NONSPECIFIC_CURRENT i, i_AMPA,i_NMDA
        POINTER rng
        RANGE synapseID, verboseLevel, deterministic
}
...
PARAMETER {
	...
	NMDA_ratio = 0.71 (1) : The ratio of NMDA to AMPA
        AMPA_scaling = 1 (1) : Scaling factor for AMPA weight, set to 0 to make purely NMDA synapse
        synapseID = 0
        verboseLevel = 0
        deterministic = 1 : 1 to make release deterministic (Pr=1), 0 to use probablistic model
}
...
NET_RECEIVE (weight,weight_AMPA, weight_NMDA, Pv, Pr, u, tsyn (ms)){
        LOCAL result
        weight_AMPA = weight * AMPA_scaling
